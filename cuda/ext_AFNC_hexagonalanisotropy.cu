...
addhexagonalanisotropy(float* __restrict__ Bx, float* __restrict__ By, float* __restrict__ Bz,
                    float* __restrict__ mx, float* __restrict__ my, float* __restrict__ mz,
                    float* __restrict__  Ms_, float  Ms_mul,
                    float* __restrict__  k3_, float  k3_mul,
                    float* __restrict__  k3b_, float  k3b_mul,
    ...
    if (i < N) {
        ...
        float  k3b = amul(k3b_, k3b_mul, i) * invMs;
        float3 u3 = normalized(vmul(c1x_, c1y_, c1z_, c1x_mul, c1y_mul, c1z_mul, i));
        float3 u1 = normalized(vmul(c2x_, c2y_, c2z_, c2x_mul, c2y_mul, c2z_mul, i));
        float3 u2 = cross(u3, u1); // 3rd axis perpendicular to u1,u2
        ...
        float3 B = 6.0f*k3*(pow5(u3m) * u3) +
                   6.0f*k3b*(5.0f*u1m*u2m*(pow3(u1m) * u1 + pow3(u2m) * u2)
                            -10.0f*pow2(u1m)*pow2(u2m)*(u1m * u2 + u2m * u1)
                            + pow5(u1m) * u2 + pow5(u2m) * u1);
        Bx[i] += B.x;
        By[i] += B.y;
        Bz[i] += B.z;
    }
