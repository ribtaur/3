...
// normalize m: m=(m1*Ms1+m2*Ms2+m3*Ms3)/Ms0
extern "C" __global__ void
normalizeAFNC(float* __restrict__ m0x, float* __restrict__ m0y, float* __restrict__ m0z,
	...
	float* __restrict__  Ms0_, float  Ms0_mul,
    ...
    if (i < N) {
        ...
        float invMs = inv_Msat(Ms0_, Ms0_mul, i);
        //printf("%g\n", invMs);
        m0x[i] = (m1x[i]*ms1+m2x[i]*ms2+m3x[i]*ms3)*invMs;
        m0y[i] = (m1y[i]*ms1+m2y[i]*ms2+m3y[i]*ms3)*invMs;
        m0z[i] = (m1z[i]*ms1+m2z[i]*ms2+m3z[i]*ms3)*invMs;
    }
}
